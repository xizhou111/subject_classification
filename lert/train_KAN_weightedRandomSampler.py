...
# 设备选择
os.environ["CUDA_VISIBLE_DEVICES"] = "2,4,5,6,7"  # 选择GPU
...
class ModelArguments:
    ...
    classifier_dropout: float = field(
        default=0.0,
        metadata={"help": "The dropout probability for the classifier layer."},
    )

@dataclass
class SubjectClassificationTrainingArguments(TrainingArguments):
    """
    Arguments pertaining to what data we are going to input our model for training and eval.
    ...
    label_smoothing_factor: Optional[float] = field(
        default=None,
        metadata={"help": "The label smoothing factor to apply (zero means no label smoothing)."},
    )
    neftune_noise_alpha: Optional[float] = field(
        default=None,
        metadata={"help": "The alpha value for Neftune noise."},
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, SubjectClassificationTrainingArguments))
    ...
    class SubjectClassificationBertModel(BertForSequenceClassification):
    ...
    model = SubjectClassificationBertModel.from_pretrained(
    ...
    model.load_state_dict(torch.load("/mnt/cfs/NLP/wyf/grade_classifier/log/2024-05-02-21-36-22/pretrain_model/roberta-chinese_0010/roberta-chinese_0010.pth"),
                          strict=False)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        classification_rep = classification_report(labels, preds, output_dict=True)
        results = {
            "accuracy": acc,
            "precision": precision,
            "recall": recall,
        ...
        # subj2id = json.load(open("/mnt/cfs/NLP/zcl/subjects_classification/category_map.json",encoding='utf-8'))
        id2subj = {0: "其他", 1: "语文", 2: "数学", 3: "英语", 4: "物理", 5: "化学", 6: "生物", 7: "历史", 8: "地理", 9: "政治"}
        # 添加每个类别的recall指标到结果字典
        for label, metrics in classification_rep.items():
            if isinstance(metrics, dict):
                # results[f'recall_{label}'] = metrics['recall']
                try:
                    results[f'recall_{id2subj[int(label)]}'] = metrics['recall']
                except:
    ...
    for label in train_dataset['label']:
        if label in label2count:
            ...
            label2count[label] = 1
    # 计算每个类别的样本权重
    weights = [1.0 / label2count[label] for label in train_dataset['label']]
    # 为每个样本设置权重
    ...
    class CustomSampler(Sampler):
        ...
        def __iter__(self):
            ...
            for i in range(0, n, 2**24):
                weights = [1.0 / label2count[label] for label in self.data_source['label'][i:i+2**24]]
                yield from torch.multinomial(torch.tensor(weights), min(self.num_samples, 2**24), self.replacement).tolist()

        def __len__(self):
            return self.num_samples

    sampler = CustomSampler(train_dataset, len(train_dataset))

    class SubjectClassificationTrainer(Trainer):
        def _get_train_sampler(self):
            return sampler
            
    # Initialize our Trainer
    trainer = SubjectClassificationTrainer(
        model=model,
        args=training_args,
